...
public class Verbalsmeltery extends BlockContainer {
	...
	public boolean onBlockActivate(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ){
		if(!world.isRemote){
			FMLNetworkHandler.openGui(player, Blocks.instance, Blocks.guiIdVerbalsmeltery, world, x, y, z);
		}
	...
	public static void updateVerbalSmelteryBlockState(boolean active, World worldObj,
			int xCoord, int yCoord, int zCoord) {
			
		int i = worldObj.getBlockMetadata(xCoord, yCoord, zCoord);
		
		TileEntity tileentity = worldObj.getBlockTileEntity(yCoord, yCoord, zCoord);
		keepInventory = true;
		
		if(active)
		{
			worldObj.setBlock(xCoord, yCoord, zCoord, Blocks.verbalsmelteryActive.blockID);
		}else{
			worldObj.setBlock(xCoord, yCoord, zCoord, Blocks.verbalsmelteryIdle.blockID);
		}
